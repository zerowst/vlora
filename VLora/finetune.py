...
from VLora.Custom import CustomLlama, CustomTrainer, CustomDataCollator
...
class DataTrainingArguments:
    """
    Arguments pertaining to what annotation_data we are going to input our model for training and eval.
    """
    ...
    train_files: Optional[List[str]] = field(default=None,
                                             metadata={"help": "The input training annotation_data file (a text file)."})
    validation_files: Optional[List[str]] = field(
        default=None,
        metadata={"help": "An optional input evaluation annotation_data file to evaluate the perplexity on (a text file)."},
    )
...
def main():
    ...
    if True:
        ...
        # If no validation annotation_data is there, validation_split_percentage will be used to divide the dataset.
    ...
    # speaker_1_id = '<|speaker-1|>'
    # speaker_2_id = '<|speaker-2|>'
    # new_tokens = [speaker_1_id, speaker_2_id]
    ...
    if len(column_names) == 1:
        ...
        input_column_name = 'input' if 'input' in column_names else column_names[0]
        second_column_name = 'second' if 'second' in column_names else column_names[1]
        target_column_name = 'target' if 'target' in column_names else column_names[2]
    else:
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        input_t = data_point[input_column_name]

        input_text = TEMPLATE_WITHOUT_SYSTEM_PROMPT.format_map({'instruction': input_t})

        target_text = data_point[target_column_name]
        target_text = target_text if isinstance(target_text, str) else target_text[0]
        ...
        tokenized_target_prompt = tokenize(target_text)
        input_prompt_len = len(tokenized_full_prompt["input_ids"]) - len(tokenized_target_prompt["input_ids"])

        tokenized_full_prompt["labels"] = [-100] * input_prompt_len + tokenized_full_prompt["input_ids"][
                                                                       input_prompt_len:]
    ...
    def vae_tokenize_prompt(data_point):
        input_t = data_point[input_column_name]
        # input_t = input_t if isinstance(input_t, str) else input_t[0]
        input_text = TEMPLATE_WITHOUT_SYSTEM_PROMPT.format_map({'instruction': input_t})

        second_t = data_point[second_column_name]
        # second_t = second_t if isinstance(second_t, str) else second_t[0]
        second_text = TEMPLATE_WITHOUT_SYSTEM_PROMPT.format_map({'instruction': second_t})

        target_text = data_point[target_column_name]
        target_text = str(target_text)


        prior_tokenized = tokenize(input_text + target_text)
        # print(len(prior_tokenized['input_ids']))
        posterior_tokenized = tokenize(second_text + target_text)

        len_gap = abs((len(prior_tokenized["input_ids"]) - len(posterior_tokenized["input_ids"])))


        if len(prior_tokenized["input_ids"]) > len(posterior_tokenized["input_ids"]):
            posterior_tokenized['input_ids'] = [tokenizer.pad_token_id] * len_gap + posterior_tokenized['input_ids']
            posterior_tokenized['attention_mask'] = [0] * len_gap + posterior_tokenized['attention_mask']
            posterior_tokenized['labels'] = [-100] * len_gap + posterior_tokenized['labels']

        else:
            prior_tokenized['input_ids'] = [tokenizer.pad_token_id] * len_gap + prior_tokenized['input_ids']

        # print(len(prior_tokenized['input_ids']))
        # print(posterior_tokenized['input_ids'])
        tokenized_target_prompt = tokenize(target_text)


        prior_len = len(prior_tokenized["input_ids"]) - len(tokenized_target_prompt["input_ids"])
        prior_tokenized["labels"] = [-100] * prior_len + prior_tokenized["input_ids"][
                                                                      prior_len:]

        posterior_len = len(posterior_tokenized["input_ids"]) - len(tokenized_target_prompt["input_ids"])
        posterior_tokenized["labels"] = [-100] * posterior_len + posterior_tokenized["input_ids"][
                                                                posterior_len:]
        posterior_tokenized['prior_ids'] = prior_tokenized['input_ids'].copy()
        # print(len(posterior_tokenized['prior_ids']))
        # print(len(posterior_tokenized['input_ids']))
        # print('\n\n')
        return posterior_tokenized

    ### rawdataset should not do batched mapping!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if training_args.do_eval:
        ...
        def compute_metrics(eval_pred):
            ...
            # .reshape(-1)
            preds = predictions[:, :-1].reshape(-1)
            metric_dict = metric.compute(predictions=preds, references=labs)

            # metric_dict.update(rouge_dict)
            return metric_dict

    # print('Posterior lora before')
    # for name, param in model.model.layers[0].self_attn.o_proj.named_parameters():
    #     if param.requires_grad == True:
    #         print(name, param.data.dtype)
    #     else:
    #         print('no grad:', name, param.data.dtype)
    ...
    # print('Posterior after lora')
    # for name, param in model_lora.model.model.layers[0].self_attn.o_proj.named_parameters():
    #     if param.requires_grad == True:
    #         print(name, param.data.dtype)
    #     else:
    #         print('no grad:', name, param.data.dtype, param.grad.dtype)
    #         param.requires_grad = True
    #
    print('lora model')
    print('Prior net')
    print(model.model)
    for name, param in model.model.prior_net.layers[0].self_attn.o_proj.named_parameters():
        if param.requires_grad == True:
            print(name, param.data.dtype)
        else:
            print('no grad:', name, param.data.dtype)
    ...
    # print('trainset:\n\n', train_datasets[1])
    # print(model(input_ids=train_datasets[1]['input_ids'], attention_mask=train_datasets[1]['attention_mask']))
    # print('trainset_len:', len(train_dataset[1]))
    trainer = Trainer(
        model=model,
        args=training_args,
    ...
    # Training
    if training_args.do_train:
        print('checkpoint:', training_args.resume_from_checkpoint)
        checkpoint = None
