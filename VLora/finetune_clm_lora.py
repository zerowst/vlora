...
class DataTrainingArguments:
    ...
    train_files: Optional[List[str]]  = field(default=None, metadata={"help": "The input training annotation_data file (a text file)."})
...
def main():
    ...
    if len(column_names)==1:
        input_column_name = "text" if "text" in column_names else column_names[0]
    ...
    def tokenize_function(examples):
        with CaptureLogger(tok_logger) as cl:
            output = tokenizer([ item for item in examples[input_column_name]],truncation=True,max_length=data_args.block_size,padding=False,return_tensors=None)
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        full_prompt = tokenizer.bos_token + input_text + tokenizer.eos_token + target_text + tokenizer.eos_token
        tokenized_full_prompt = tokenize(full_prompt)

        # if not train_on_inputs:
        user_prompt = tokenizer.bos_token + input_text + tokenizer.eos_token
        ...
        tokenized_full_prompt["labels"] = [-100] * (user_prompt_len+1) + tokenized_full_prompt["labels"][user_prompt_len+1:]
