...
def count_depth_of_dictionary(dictionary):
    if not isinstance(dictionary, dict) or not dictionary:
        return 0
    return 1 + max(count_depth_of_dictionary(dictionary[key]) for key in dictionary)
    ...
    for dialog in data.values():
        ...
        for turn in dialog:
            ...
            for domain in turn["state"]:
                if count_depth_of_dictionary(turn["state"][domain]) > 1:
                    # turn["state"] = turn["state"][domain]
                    to_del = [slot for slot, val in turn["state"][domain].items() if isinstance(val, dict)]
                    for slot in to_del:
                        del turn["state"][domain][slot]
...
def load_goals():
    ...
    with open(os.path.join(dir_path, "annotation_data", "goals.json")) as f:
        return json.load(f)


def load_booked_domains():
    dir_path = os.path.dirname(os.path.realpath(__file__))
    with open(os.path.join(dir_path, "annotation_data", "booked_domains.json")) as f:
        return json.load(f)
...
def load_references(systems=['mwz22']): #, 'damd', 'uniconv', 'hdsa', 'lava', 'augpt']):
    ...
    for system in systems:
        if system == 'mwz22':
            continue
        dir_path = os.path.dirname(os.path.realpath(__file__))
        with open(os.path.join(dir_path, "annotation_data", "references", f"{system}.json")) as f:
...
def load_multiwoz22_reference():
    dir_path = os.path.dirname(os.path.realpath(__file__))
    data_path = os.path.join(dir_path, "annotation_data", "references", "mwz22.json")
...
def load_gold_states():
    dir_path = os.path.dirname(os.path.realpath(__file__))
    data_path = os.path.join(dir_path, "annotation_data", "gold_states.json")
    if os.path.exists(data_path):
        with open(data_path) as f:
            return json.load(f)
...
def load_multiwoz22():
    ...
    dir_path = os.path.dirname(os.path.realpath(__file__))
    reference_path = os.path.join(dir_path, "annotation_data", "references", "mwz22.json")
    state_path = os.path.join(dir_path, "annotation_data", "gold_states.json")
